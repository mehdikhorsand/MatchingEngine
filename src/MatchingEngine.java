...
public class MatchingEngine {
    ...
    public void replace_order_request(int old_order_id, Order new_order) {
        ...
        if (old_order != null && old_order.broker_id.id == new_order.broker_id.id
                && old_order.shareholder_id.id == new_order.shareholder_id.id
                && old_order.is_buy == new_order.is_buy
                && new_order.min_qty == 0) {
    ...
    public String add_order(Order order){
        ...
        if(order.has_valid_attrs() && environment.validate_order_price_limit(order)
            ...
            if(total_traded_qty < order.min_qty || (order.fill_and_kill && total_traded_qty == 0)) {
                // order is eliminated.
                rollback_by_trades(first_state);
                return "Eliminated";
            }
    ...
    public void rollback_by_trades(String first_state) {
        String trades_string = print_trades();
        while(trades.size() > 0) {
        ...
        String after_rollback = toString();
        assert Objects.equals(after_rollback, first_state):
                "\n" + trades_string +
                "\nROLLBACK ERROR:" +
                "\nexpected:\n" + first_state +
                "\noutput:\n" + after_rollback;
